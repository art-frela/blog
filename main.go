...
func main() {
	connSTR := flag.String("c", os.Getenv("DATABASE_URL"), "MySQL connection string, format: user:password@tcp(host:port)/database, or use ENV: export DATABASE_URL=root:master@tcp(localhost:3306)/blog")
