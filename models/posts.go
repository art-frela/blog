...
type Post struct {
	...
	RubricID     null.String `boil:"rubric_id" json:"rubric_id,omitempty" toml:"rubric_id" yaml:"rubric_id,omitempty"`
	...
	ParentPostID null.String `boil:"parent_post_id" json:"parent_post_id,omitempty" toml:"parent_post_id" yaml:"parent_post_id,omitempty"`
	CountOfViews int         `boil:"count_of_views" json:"count_of_views" toml:"count_of_views" yaml:"count_of_views"`
	CountOfStars int         `boil:"count_of_stars" json:"count_of_stars" toml:"count_of_stars" yaml:"count_of_stars"`
	CommentsIds  null.JSON   `boil:"comments_ids" json:"comments_ids,omitempty" toml:"comments_ids" yaml:"comments_ids,omitempty"`
...
var PostWhere = struct {
	...
	RubricID     whereHelpernull_String
	Tags         whereHelpernull_JSON
	State        whereHelpernull_String
	Content      whereHelperstring
	CreatedAt    whereHelpernull_Time
	ModifiedAt   whereHelpernull_Time
	ParentPostID whereHelpernull_String
	CountOfViews whereHelperint
	CountOfStars whereHelperint
	CommentsIds  whereHelpernull_JSON
}{
	ID:           whereHelperstring{field: "`posts`.`id`"},
	Title:        whereHelperstring{field: "`posts`.`title`"},
	AuthorID:     whereHelpernull_String{field: "`posts`.`author_id`"},
	RubricID:     whereHelpernull_String{field: "`posts`.`rubric_id`"},
	Tags:         whereHelpernull_JSON{field: "`posts`.`tags`"},
	State:        whereHelpernull_String{field: "`posts`.`state`"},
	Content:      whereHelperstring{field: "`posts`.`content`"},
	CreatedAt:    whereHelpernull_Time{field: "`posts`.`created_at`"},
	ModifiedAt:   whereHelpernull_Time{field: "`posts`.`modified_at`"},
	ParentPostID: whereHelpernull_String{field: "`posts`.`parent_post_id`"},
	CountOfViews: whereHelperint{field: "`posts`.`count_of_views`"},
	CountOfStars: whereHelperint{field: "`posts`.`count_of_stars`"},
	CommentsIds:  whereHelpernull_JSON{field: "`posts`.`comments_ids`"},
}
...
var (
	postAllColumns            = []string{"id", "title", "author_id", "rubric_id", "tags", "state", "content", "created_at", "modified_at", "parent_post_id", "count_of_views", "count_of_stars", "comments_ids"}
	postColumnsWithoutDefault = []string{"id", "title", "author_id", "rubric_id", "tags", "state", "content", "parent_post_id", "comments_ids"}
	postColumnsWithDefault    = []string{"created_at", "modified_at", "count_of_views", "count_of_stars"}
...
// Rubric pointed to by the foreign key.
func (o *Post) Rubric(mods ...qm.QueryMod) rubricQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`id` = ?", o.RubricID),
	}

	queryMods = append(queryMods, mods...)

	query := Rubrics(queryMods...)
	queries.SetFrom(query.Query, "`rubrics`")
...
// LoadRubric allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (postL) LoadRubric(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	var slice []*Post
	var object *Post
	...
	if singular {
		...
		if !queries.IsNil(object.RubricID) {
			args = append(args, object.RubricID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.RubricID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.RubricID) {
				args = append(args, obj.RubricID)
			}
	...
	query := NewQuery(qm.From(`rubrics`), qm.WhereIn(`rubrics.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load Rubric")
	}

	var resultSlice []*Rubric
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice Rubric")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for rubrics")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for rubrics")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.RubricID, foreign.ID) {
				local.R.Rubric = foreign
				if foreign.R == nil {
					foreign.R = &rubricR{}
				}
...
// SetRubric of the post to the related item.
// Sets o.R.Rubric to related.
// Adds o to related.R.Posts.
func (o *Post) SetRubric(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Rubric) error {
	var err error
	if insert {
	...
	queries.Assign(&o.RubricID, related.ID)
	if o.R == nil {
		o.R = &postR{
			Rubric: related,
		}
...
// RemoveRubric relationship.
// Sets o.R.Rubric to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *Post) RemoveRubric(ctx context.Context, exec boil.ContextExecutor, related *Rubric) error {
	var err error

	queries.SetScanner(&o.RubricID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("rubric_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.Posts {
		if queries.Equal(o.RubricID, ri.RubricID) {
			continue
		}
...
// Removes o from all passed in related items' relationships struct (Optional).
func (o *Post) RemoveAuthor(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	var err error
