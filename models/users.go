...
type User struct {
	...
	UserRole   int         `boil:"user_role" json:"user_role" toml:"user_role" yaml:"user_role"`
...
var UserWhere = struct {
	...
	UserRole:   whereHelperint{field: "`users`.`user_role`"},
...
var (
	userAllColumns            = []string{"id", "username", "nick", "email", "created_at", "modified_at", "user_role", "salt", "avatar_url"}
	userColumnsWithoutDefault = []string{"id", "username", "nick", "email", "avatar_url"}
	userColumnsWithDefault    = []string{"created_at", "modified_at", "user_role", "salt"}
...
func (o *User) AuthorComments(mods ...qm.QueryMod) commentQuery {
...
func (userL) LoadAuthorComments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
...
func (o *User) AddAuthorComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	var err error
...
// SetAuthorComments removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Author's AuthorComments accordingly.
// Replaces o.R.AuthorComments with related.
// Sets related.R.Author's AuthorComments accordingly.
func (o *User) SetAuthorComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	query := "update `comments` set `author_id` = null where `author_id` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.AuthorComments {
		...
		o.R.AuthorComments = nil
	}
	return o.AddAuthorComments(ctx, exec, insert, related...)
}

// RemoveAuthorComments relationships from objects passed in.
// Removes related items from R.AuthorComments (uses pointer comparison, removal does not keep order)
// Sets related.R.Author.
func (o *User) RemoveAuthorComments(ctx context.Context, exec boil.ContextExecutor, related ...*Comment) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.AuthorID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.AuthorComments {
			if rel != ri {
				continue
			}

			ln := len(o.R.AuthorComments)
			if ln > 1 && i < ln-1 {
				o.R.AuthorComments[i] = o.R.AuthorComments[ln-1]
			}
			o.R.AuthorComments = o.R.AuthorComments[:ln-1]
			break
		}
	}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Author's AuthorPosts accordingly.
// Replaces o.R.AuthorPosts with related.
// Sets related.R.Author's AuthorPosts accordingly.
func (o *User) SetAuthorPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	query := "update `posts` set `author_id` = null where `author_id` = ?"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
...
// Sets related.R.Author.
func (o *User) RemoveAuthorPosts(ctx context.Context, exec boil.ContextExecutor, related ...*Post) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.AuthorID, nil)
		if rel.R != nil {
