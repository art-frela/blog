...
func testCommentToOneUserUsingAuthor(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CommentSlice{&local}
	if err = local.L.LoadAuthor(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCommentToOneSetOpUserUsingAuthor(t *testing.T) {
	var err error
...
func testCommentToOneRemoveOpUserUsingAuthor(t *testing.T) {
	var err error
...
var (
	commentDBTypes = map[string]string{`ID`: `varchar`, `AuthorID`: `varchar`, `Content`: `text`, `CountOfStars`: `int`, `PostID`: `varchar`}
	_              = bytes.MinRead
)
