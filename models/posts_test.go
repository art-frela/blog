...
func testPostToOneRubricUsingRubric(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, rubricDBTypes, false, rubricColumnsWithDefault...); err != nil {
	...
	queries.Assign(&local.RubricID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Rubric().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadRubric(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Rubric == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Rubric = nil
	if err = local.L.LoadRubric(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Rubric == nil {
		t.Error("struct should have been eager loaded")
...
func testPostToOneSetOpRubricUsingRubric(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, rubricDBTypes, false, strmangle.SetComplement(rubricPrimaryKeyColumns, rubricColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, rubricDBTypes, false, strmangle.SetComplement(rubricPrimaryKeyColumns, rubricColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Rubric{&b, &c} {
		err = a.SetRubric(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.RubricID, x.ID) {
			t.Error("foreign key was wrong value", a.RubricID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.RubricID))
		reflect.Indirect(reflect.ValueOf(&a.RubricID)).Set(zero)
		...
		if !queries.Equal(a.RubricID, x.ID) {
			t.Error("foreign key was wrong value", a.RubricID, x.ID)
		}
	}
}

func testPostToOneRemoveOpRubricUsingRubric(t *testing.T) {
	var err error
	...
	if err = a.SetRubric(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveRubric(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Rubric().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.RubricID) {
...
func testPostToOneRemoveOpUserUsingAuthor(t *testing.T) {
	var err error
...
var (
	postDBTypes = map[string]string{`ID`: `varchar`, `Title`: `varchar`, `AuthorID`: `varchar`, `RubricID`: `varchar`, `Tags`: `json`, `State`: `set`, `Content`: `text`, `CreatedAt`: `datetime`, `ModifiedAt`: `datetime`, `ParentPostID`: `varchar`, `CountOfViews`: `int`, `CountOfStars`: `int`, `CommentsIds`: `json`}
	_           = bytes.MinRead
)
